// Bilingual UI strings. Default language is Farsi; the user can switch
// with the toggle in the navbar and the choice persists in localStorage.

const I18N = {
    en: {
        nav_home: 'Home',
        nav_cities: 'Cities',
        nav_info: 'Info',
        nav_compare: 'Compare',
        nav_news: 'News',
        news_title: 'News & Updates',
        news_lead: 'Announcements about the project: new features, new cities, and changes to how the data is collected.',
        news_empty_title: 'No news yet',
        news_empty_body: 'We are just getting started. Updates about new features and cities will be posted here soon.',
        news_error: 'Could not load the news right now. Please try again in a moment.',
        news_read_more: 'Read more',
        news_read_less: 'Show less',
        share: 'Share',
        share_title: 'Share this',
        share_card_alt: 'Shareable image of the current values',
        share_download: 'Download image',
        share_share: 'Share',
        share_preparing: 'Preparing the image...',
        share_failed: 'The image could not be created on this browser.',
        share_hint: 'Save the image and share it anywhere - the values are the ones shown on this page right now.',
        share_saved: 'Image saved.',
        coming_soon: 'Buy me a coffee - coming soon!',
        donate_short: 'Support',
        donate_secure: 'Payment is handled by Zibal. We never see or store your card details.',
        donate_title: 'Buy me a coffee',
        donate_intro: '"Buy me a coffee" is a simple way to support HavaChetor. If the site is useful to you, you can chip in the price of a coffee to help keep it running.',
        donate_where: 'Every contribution goes only toward developing new features, paying for the servers that collect the data around the clock, and covering maintenance and running costs. HavaChetor has no ads and no paywall.',
        donate_pick: 'Choose an amount',
        donate_continue: 'Continue to payment',
        donate_close: 'Close',
        donate_soon: 'Thank you! Online payment will be available very soon.',
        toman: 'Toman',
        first_name: 'First name',
        last_name: 'Last name',
        donate_name_required: 'Please enter your first and last name.',
        donate_redirecting: 'Redirecting to payment...',
        donate_error: 'Could not start the payment. Please try again in a moment.',
        back_home: 'Back to home',
        pay_success_title: 'Thank you!',
        pay_success_msg: 'Your payment was successful. Your support keeps HavaChetor running and growing.',
        pay_pending_title: 'Payment is being checked',
        pay_pending_msg: 'We received your payment but could not confirm it with the gateway yet. If the amount was deducted it will be settled shortly.',
        pay_failed_title: 'Payment was not completed',
        pay_failed_msg: 'The payment was cancelled or did not go through. No amount has been charged. You are welcome to try again.',
        pay_amount: 'Amount:',
        pay_ref: 'Reference number:',
        pay_track: 'Tracking id:',
        map_title: 'Live Map',
        map_note: 'Every dot is a tracked city. Hover for the latest conditions; click to open its dashboard.',
        compare_title: 'Compare Cities',
        compare_pick: 'Pick 2 to 4 cities and see their temperature and wind side by side.',
        compare_btn: 'Compare',
        compare_table_title: 'Side by Side',
        compare_need_more: 'At least two cities are needed for a comparison.',
        compare_no_data: 'The selected cities have no collected data yet. Once the collector gathers snapshots, comparisons will appear here.',
        retry: 'Try again',
        metric: 'Metric',
        no_data_title: 'No data collected yet',
        no_data_body: 'This city has no weather snapshots stored yet.',
        collector_stopped: 'The data collector is not running - check the API and admin panel.',
        collector_soon: 'The collector runs every {min} minutes; data should appear shortly.',
        last_error: 'Last collection error:',
        theme_toggle: 'Toggle light/dark theme',
        default_tagline: 'Weather data collection and analysis, city by city',
        default_intro: 'We continuously collect current weather conditions for cities around the world and turn them into clear, comparable analytics. Pick a city to explore its temperature and wind behaviour over time.',
        default_examples: 'For every city we track average temperature, temperature range and rate of change, wind speed and direction, calm periods, and more - all computed from data we collect ourselves at regular intervals.',
        default_about_title: 'About This Project',
        default_about: 'This project is an independent weather data pipeline. We fetch current conditions for each tracked city on a fixed schedule, store every snapshot, and compute analytics over the collected history.',
        default_mission: 'Our goal is to make raw weather history accessible and understandable. All figures on this site come from our own collected snapshots.',
        default_data_desc: 'For each city we store the current temperature, wind speed, wind direction, day/night flag and weather code, together with a timestamp.',
        default_footer: 'Weather Analysis System',
        footer_by: 'Built and run by',
        footer_and: 'and',
        footer_ideas_title: 'Have an idea for the site?',
        footer_ideas_text: 'A feature you would like to see, a city you want us to track, or something that could work better - tell us and we will look at it.',
        footer_ideas_cta: 'Send us your idea',
        footer_ideas_subject: 'Idea for the site',
        explore_cities: 'Explore Cities',
        about_project: 'About the Project',
        cities_we_track: 'Cities We Track',
        see_all: 'See all',
        country_filter: 'Filter by country',
        country_all: 'All',
        no_cities_country: 'No tracked cities in this country yet.',
        cities_more: '+{n} more on the Cities page',
        what_you_find: 'What You Will Find Here',
        temp_analytics: 'Temperature Analytics',
        temp_analytics_desc: 'Average temperature, minimum and maximum range, rate of change and hourly delta, computed over the most recent records of each city.',
        wind_analytics: 'Wind Analytics',
        wind_analytics_desc: 'Average and peak wind speed, dominant direction, direction variability and calm-period detection with a configurable threshold.',
        raw_history: 'Raw History',
        raw_history_desc: 'Every snapshot we collect is stored and browsable, so you can see exactly how conditions evolved over time.',
        how_it_works: 'How It Works',
        loading: 'Loading...',
        error_cities: 'Could not load cities. Please check if the API is running.',
        error_city: 'Could not load this city. It may not exist, or the API is not running.',
        error_content: 'Could not load site content. Please check if the API is running.',
        no_cities: 'No cities are being tracked yet.',
        no_data_yet: 'No data yet',
        latest_temperature: 'latest temperature',
        wind_label: 'Wind:',
        records_label: 'Records collected:',
        last_update: 'Last update:',
        cities_title: 'Cities',
        pick_city: 'Pick a city to open its dashboard with temperature, wind and raw data analytics.',
        summary: 'Summary',
        temperature: 'Temperature',
        wind: 'Wind',
        recent_data: 'Recent Data',
        period_label: 'Period (records):',
        refresh: 'Refresh',
        average: 'Average',
        range: 'Range',
        rate_of_change: 'Rate of Change',
        delta_per_hour: 'Delta per Hour',
        avg_speed: 'Average Speed',
        peak_speed: 'Peak Speed',
        dominant_direction: 'Dominant Direction',
        direction_variability: 'Direction Variability',
        calm_periods: 'Calm Periods',
        threshold_label: 'Threshold (km/h):',
        calculate: 'Calculate',
        calm_count_label: 'Calm periods:',
        total_periods_label: 'Total periods:',
        calm_pct_label: 'Calm percentage:',
        rows_label: 'Rows:',
        load: 'Load',
        total_records_label: 'Total records:',
        showing_label: 'Showing:',
        th_id: 'ID',
        th_time: 'Time',
        th_temp: 'Temperature (C)',
        th_wind: 'Wind (km/h)',
        th_dir: 'Direction',
        th_code: 'Code',
        th_daynight: 'Day/Night',
        day: 'Day',
        night: 'Night',
        min_label: 'Min:',
        max_label: 'Max:',
        range_label: 'Range:',
        peak_label: 'Peak:',
        deg_c: 'degrees C',
        deg_c_avg: 'degrees C average',
        deg_c_per_hour: 'degrees C / hour',
        kmh: 'km/h',
        kmh_avg: 'km/h average',
        degrees: 'degrees',
        std_dev: 'std deviation',
        pct_calm: '% calm time',
        analyzed: 'analyzed',
        data_points: 'Data Points',
        wind_speed: 'Wind Speed',
        wind_direction: 'Wind Direction',
        records_collected: 'records collected',
        who_we_are: 'Who We Are',
        our_mission: 'Our Mission',
        data_we_collect: 'The Data We Collect',
        contact: 'Contact',
        contact_line: 'Questions or suggestions? Reach us at',
        developers: 'Developers',
        dev_mehrshad: 'Mehrshad Asadi',
        dev_sepehr: 'Sepehr Sedigh',
        linkedin: 'LinkedIn',
        trends: 'Trends',
        temp_trend: 'Temperature over time',
        wind_trend: 'Wind speed over time',
        chart_empty: 'Not enough data to chart yet.',
        records_milestones: 'Records & Milestones',
        records_note: "All-time highlights computed over this city's full collected history.",
        hottest: 'Hottest',
        coldest: 'Coldest',
        windiest: 'Windiest',
        longest_calm: 'Longest Calm Streak',
        consecutive_records: 'consecutive records',
        wettest: 'Wettest',
        most_humid: 'Most Humid',
        feels_like: 'Feels Like',
        humidity: 'Humidity',
        precipitation: 'Precipitation',
        pressure: 'Pressure',
        pct: '% average',
        mm_total: 'mm total',
        hpa: 'hPa average',
        air_quality: 'Air Quality',
        aqi: 'Air Quality Index',
        avg_aqi: 'Average AQI',
        aqi_scale: 'scale 1 (good) to 5 (very poor)',
        aqi_trend: 'Air Quality Index over time',
        pm25_trend: 'PM2.5 over time',
        avg_ugm3: 'µg/m³ average',
        aqi_good: 'Good',
        aqi_fair: 'Fair',
        aqi_moderate: 'Moderate',
        aqi_poor: 'Poor',
        aqi_very_poor: 'Very Poor',
        pollutants: 'Pollutants',
        clouds: 'Clouds',
        precip_sky: 'Precipitation & Sky',
        precip_trend: 'Precipitation',
        humidity_trend: 'Humidity & cloud cover',
        no_precip: 'No precipitation recorded in this window.',
        forecast: 'Short-term Trend Estimate',
        temp_forecast: 'Temperature — next hours',
        aqi_forecast: 'Air Quality Index — next hours',
        forecast_note: 'A naive projection of recent readings, not a meteorological forecast.',
        patterns: 'Patterns by Hour & Day',
        patterns_note: 'Average by hour of day (columns) and day of week (rows), across all collected history.',
        temp_pattern: 'Temperature pattern',
        aqi_pattern: 'Air Quality Index pattern',
        health_advice: 'Health Advice',
        main_pollutant: 'Main pollutant right now: {p}.',
        aqi_advice_1: 'Air quality is good. It is a great time for outdoor activity for everyone.',
        aqi_advice_2: 'Air quality is fair. Unusually sensitive people should consider limiting prolonged outdoor exertion.',
        aqi_advice_3: 'Air quality is moderate. Sensitive groups (children, elderly, those with heart or lung conditions) should limit prolonged outdoor exertion.',
        aqi_advice_4: 'Air quality is poor. Everyone should reduce prolonged or heavy outdoor exertion; sensitive groups should stay indoors.',
        aqi_advice_5: 'Air quality is very poor. Avoid outdoor activity, keep windows closed, and use a mask or air purifier if you must go out.',
        weekdays: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
    },
    fa: {
        nav_home: 'خانه',
        nav_cities: 'شهرها',
        nav_info: 'درباره',
        nav_compare: 'مقایسه',
        nav_news: 'اخبار',
        news_title: 'اخبار و به‌روزرسانی‌ها',
        news_lead: 'اطلاعیه‌های پروژه: امکانات تازه، شهرهای جدید و تغییرات مربوط به شیوه جمع‌آوری داده‌ها.',
        news_empty_title: 'هنوز خبری نیست',
        news_empty_body: 'تازه شروع کرده‌ایم. به‌زودی خبرهای مربوط به امکانات و شهرهای جدید اینجا منتشر می‌شود.',
        news_error: 'در حال حاضر امکان دریافت اخبار نبود. لطفاً کمی بعد دوباره تلاش کنید.',
        news_read_more: 'ادامه مطلب',
        news_read_less: 'بستن',
        share: 'اشتراک‌گذاری',
        share_title: 'اشتراک‌گذاری',
        share_card_alt: 'تصویر قابل اشتراک‌گذاری از مقادیر فعلی',
        share_download: 'دانلود تصویر',
        share_share: 'اشتراک‌گذاری',
        share_preparing: 'در حال ساخت تصویر...',
        share_failed: 'ساخت تصویر در این مرورگر ممکن نشد.',
        share_hint: 'تصویر را ذخیره کنید و هر جا خواستید به اشتراک بگذارید - مقادیر همان چیزی است که همین حالا در این صفحه می‌بینید.',
        share_saved: 'تصویر ذخیره شد.',
        coming_soon: 'یک قهوه مهمانم کن - به‌زودی!',
        donate_short: 'حمایت',
        donate_secure: 'پرداخت از طریق درگاه زیبال انجام می‌شود. اطلاعات کارت شما هرگز نزد ما ذخیره نمی‌شود.',
        donate_title: 'یک قهوه مهمانم کن',
        donate_intro: '«یک قهوه مهمانم کن» راهی ساده برای حمایت از هوا چطور است. اگر این سایت برایتان مفید بوده، می‌توانید به اندازه یک فنجان قهوه کمک کنید تا سرِپا بماند.',
        donate_where: 'همه کمک‌ها فقط صرف توسعه امکانات جدید، هزینه سرورهایی که شبانه‌روز داده‌ها را جمع‌آوری می‌کنند و نگهداری و هزینه‌های جاری سایت می‌شود. هوا چطور هیچ تبلیغات و هیچ بخش پولی اجباری ندارد.',
        donate_pick: 'یک مبلغ را انتخاب کنید',
        donate_continue: 'ادامه پرداخت',
        donate_close: 'بستن',
        donate_soon: 'سپاسگزاریم! پرداخت آنلاین به‌زودی فعال می‌شود.',
        toman: 'تومان',
        first_name: 'نام',
        last_name: 'نام خانوادگی',
        donate_name_required: 'لطفاً نام و نام خانوادگی خود را وارد کنید.',
        donate_redirecting: 'در حال انتقال به درگاه پرداخت...',
        donate_error: 'شروع پرداخت ممکن نشد. لطفاً کمی بعد دوباره تلاش کنید.',
        back_home: 'بازگشت به خانه',
        pay_success_title: 'سپاسگزاریم!',
        pay_success_msg: 'پرداخت شما با موفقیت انجام شد. حمایت شما باعث می‌شود هوا چطور فعال بماند و بهتر شود.',
        pay_pending_title: 'در حال بررسی پرداخت',
        pay_pending_msg: 'پرداخت شما دریافت شد اما هنوز از سوی درگاه تایید نشده است. در صورت کسر وجه، به‌زودی تعیین‌تکلیف می‌شود.',
        pay_failed_title: 'پرداخت انجام نشد',
        pay_failed_msg: 'پرداخت لغو شد یا ناموفق بود و مبلغی از حساب شما کسر نشده است. می‌توانید دوباره تلاش کنید.',
        pay_amount: 'مبلغ:',
        pay_ref: 'شماره پیگیری بانک:',
        pay_track: 'شناسه تراکنش:',
        map_title: 'نقشه زنده',
        map_note: 'هر نقطه یک شهر رصدشده است. برای دیدن آخرین وضعیت، نشانگر را روی آن ببرید و برای باز کردن داشبورد کلیک کنید.',
        compare_title: 'مقایسه شهرها',
        compare_pick: 'دو تا چهار شهر را انتخاب کنید و دما و باد آن‌ها را کنار هم ببینید.',
        compare_btn: 'مقایسه',
        compare_table_title: 'کنار هم',
        compare_need_more: 'برای مقایسه دست‌کم دو شهر لازم است.',
        compare_no_data: 'شهرهای انتخاب‌شده هنوز داده‌ای ندارند. پس از جمع‌آوری داده‌ها، مقایسه اینجا نمایش داده می‌شود.',
        retry: 'تلاش دوباره',
        metric: 'شاخص',
        no_data_title: 'هنوز داده‌ای جمع‌آوری نشده',
        no_data_body: 'برای این شهر هنوز هیچ رکورد آب‌وهوایی ذخیره نشده است.',
        collector_stopped: 'جمع‌کننده داده در حال اجرا نیست - سرویس و پنل مدیریت را بررسی کنید.',
        collector_soon: 'جمع‌کننده هر {min} دقیقه اجرا می‌شود؛ داده‌ها به‌زودی نمایش داده می‌شوند.',
        last_error: 'آخرین خطای جمع‌آوری:',
        theme_toggle: 'تغییر حالت روشن/تاریک',
        default_tagline: 'جمع‌آوری و تحلیل داده‌های هواشناسی، شهر به شهر',
        default_intro: 'ما به‌طور پیوسته وضعیت آب‌وهوای شهرهای مختلف جهان را جمع‌آوری می‌کنیم و آن را به تحلیل‌های روشن و قابل مقایسه تبدیل می‌کنیم. یک شهر را انتخاب کنید تا رفتار دما و باد آن را در طول زمان ببینید.',
        default_examples: 'برای هر شهر میانگین دما، بازه دما و نرخ تغییر، سرعت و جهت باد، دوره‌های آرام و موارد دیگر را دنبال می‌کنیم - همه از داده‌هایی که خودمان در بازه‌های منظم جمع‌آوری می‌کنیم.',
        default_about_title: 'درباره این پروژه',
        default_about: 'این پروژه یک سامانه مستقل جمع‌آوری داده‌های هواشناسی است. ما وضعیت لحظه‌ای هر شهر را طبق برنامه زمان‌بندی دریافت می‌کنیم، هر رکورد را ذخیره می‌کنیم و روی تاریخچه جمع‌آوری‌شده تحلیل انجام می‌دهیم.',
        default_mission: 'هدف ما دسترس‌پذیر و قابل فهم کردن تاریخچه خام آب‌وهواست. همه اعداد این سایت از رکوردهایی که خودمان جمع کرده‌ایم به دست می‌آید.',
        default_data_desc: 'برای هر شهر دمای فعلی، سرعت باد، جهت باد، شاخص روز/شب و کد وضعیت هوا را همراه با برچسب زمانی ذخیره می‌کنیم.',
        default_footer: 'سامانه تحلیل آب‌وهوا',
        footer_by: 'ساخته و نگهداری می‌شود توسط',
        footer_and: 'و',
        footer_ideas_title: 'ایده‌ای برای سایت دارید؟',
        footer_ideas_text: 'امکانی که دوست دارید اضافه شود، شهری که می‌خواهید رصد کنیم، یا هر چیزی که می‌تواند بهتر کار کند - به ما بگویید تا بررسی کنیم.',
        footer_ideas_cta: 'ایده‌تان را برای ما بفرستید',
        footer_ideas_subject: 'ایده برای سایت',
        explore_cities: 'مشاهده شهرها',
        about_project: 'درباره پروژه',
        cities_we_track: 'شهرهایی که رصد می‌کنیم',
        see_all: 'مشاهده همه',
        country_filter: 'فیلتر بر اساس کشور',
        country_all: 'همه',
        no_cities_country: 'هنوز شهری از این کشور رصد نمی‌شود.',
        cities_more: '{n} شهر دیگر در صفحه شهرها',
        what_you_find: 'اینجا چه چیزهایی خواهید یافت',
        temp_analytics: 'تحلیل دما',
        temp_analytics_desc: 'میانگین دما، کمینه و بیشینه، نرخ تغییر و دلتای ساعتی، محاسبه‌شده روی جدیدترین رکوردهای هر شهر.',
        wind_analytics: 'تحلیل باد',
        wind_analytics_desc: 'سرعت میانگین و بیشینه باد، جهت غالب، تغییرپذیری جهت و تشخیص دوره‌های آرام با آستانه قابل تنظیم.',
        raw_history: 'تاریخچه خام',
        raw_history_desc: 'هر رکوردی که جمع‌آوری می‌کنیم ذخیره و قابل مشاهده است تا دقیقاً ببینید شرایط در طول زمان چگونه تغییر کرده است.',
        how_it_works: 'چگونه کار می‌کند',
        loading: 'در حال بارگذاری...',
        error_cities: 'بارگذاری شهرها ممکن نشد. لطفاً از در دسترس بودن سرویس مطمئن شوید.',
        error_city: 'بارگذاری این شهر ممکن نشد. ممکن است وجود نداشته باشد یا سرویس در دسترس نباشد.',
        error_content: 'بارگذاری محتوای سایت ممکن نشد. لطفاً از در دسترس بودن سرویس مطمئن شوید.',
        no_cities: 'هنوز شهری رصد نمی‌شود.',
        no_data_yet: 'هنوز داده‌ای نیست',
        latest_temperature: 'آخرین دما',
        wind_label: 'باد:',
        records_label: 'رکوردهای جمع‌آوری‌شده:',
        last_update: 'آخرین به‌روزرسانی:',
        cities_title: 'شهرها',
        pick_city: 'یک شهر را انتخاب کنید تا داشبورد دما، باد و داده‌های خام آن را ببینید.',
        summary: 'خلاصه',
        temperature: 'دما',
        wind: 'باد',
        recent_data: 'داده‌های اخیر',
        period_label: 'بازه (تعداد رکورد):',
        refresh: 'به‌روزرسانی',
        average: 'میانگین',
        range: 'بازه تغییرات',
        rate_of_change: 'نرخ تغییر',
        delta_per_hour: 'دلتا در ساعت',
        avg_speed: 'سرعت میانگین',
        peak_speed: 'سرعت بیشینه',
        dominant_direction: 'جهت غالب',
        direction_variability: 'تغییرپذیری جهت',
        calm_periods: 'دوره‌های آرام',
        threshold_label: 'آستانه (کیلومتر بر ساعت):',
        calculate: 'محاسبه',
        calm_count_label: 'دوره‌های آرام:',
        total_periods_label: 'کل دوره‌ها:',
        calm_pct_label: 'درصد آرام:',
        rows_label: 'تعداد ردیف:',
        load: 'بارگذاری',
        total_records_label: 'کل رکوردها:',
        showing_label: 'نمایش:',
        th_id: 'شناسه',
        th_time: 'زمان',
        th_temp: 'دما (سانتی‌گراد)',
        th_wind: 'باد (km/h)',
        th_dir: 'جهت',
        th_code: 'کد',
        th_daynight: 'روز/شب',
        day: 'روز',
        night: 'شب',
        min_label: 'کمینه:',
        max_label: 'بیشینه:',
        range_label: 'بازه:',
        peak_label: 'بیشینه:',
        deg_c: 'درجه سانتی‌گراد',
        deg_c_avg: 'میانگین، درجه سانتی‌گراد',
        deg_c_per_hour: 'درجه سانتی‌گراد در ساعت',
        kmh: 'کیلومتر بر ساعت',
        kmh_avg: 'میانگین، کیلومتر بر ساعت',
        degrees: 'درجه',
        std_dev: 'انحراف معیار',
        pct_calm: 'درصد زمان آرام',
        analyzed: 'تحلیل‌شده',
        data_points: 'تعداد داده',
        wind_speed: 'سرعت باد',
        wind_direction: 'جهت باد',
        records_collected: 'رکورد جمع‌آوری‌شده',
        who_we_are: 'ما که هستیم',
        our_mission: 'ماموریت ما',
        data_we_collect: 'داده‌هایی که جمع می‌کنیم',
        contact: 'تماس',
        contact_line: 'پرسش یا پیشنهادی دارید؟ با ما در تماس باشید:',
        developers: 'توسعه‌دهندگان',
        dev_mehrshad: 'مهرشاد اسدی',
        dev_sepehr: 'سپهر صدیق',
        linkedin: 'لینکدین',
        trends: 'روند تغییرات',
        temp_trend: 'دما در طول زمان',
        wind_trend: 'سرعت باد در طول زمان',
        chart_empty: 'هنوز داده کافی برای رسم نمودار وجود ندارد.',
        records_milestones: 'رکوردها و نقاط عطف',
        records_note: 'برجسته‌ترین رکوردها که روی کل تاریخچه جمع‌آوری‌شده این شهر محاسبه شده است.',
        hottest: 'گرم‌ترین',
        coldest: 'سردترین',
        windiest: 'بادخیزترین',
        longest_calm: 'طولانی‌ترین دوره آرام',
        consecutive_records: 'رکورد متوالی',
        wettest: 'پربارش‌ترین',
        most_humid: 'مرطوب‌ترین',
        feels_like: 'دمای محسوس',
        humidity: 'رطوبت',
        precipitation: 'بارش',
        pressure: 'فشار هوا',
        pct: 'میانگین (درصد)',
        mm_total: 'مجموع (میلی‌متر)',
        hpa: 'میانگین (هکتوپاسکال)',
        air_quality: 'کیفیت هوا',
        aqi: 'شاخص کیفیت هوا',
        avg_aqi: 'میانگین شاخص کیفیت هوا',
        aqi_scale: 'مقیاس ۱ (خوب) تا ۵ (بسیار ناسالم)',
        aqi_trend: 'شاخص کیفیت هوا در طول زمان',
        pm25_trend: 'ذرات معلق PM2.5 در طول زمان',
        avg_ugm3: 'میانگین (میکروگرم بر مترمکعب)',
        aqi_good: 'خوب',
        aqi_fair: 'قابل قبول',
        aqi_moderate: 'متوسط',
        aqi_poor: 'ناسالم',
        aqi_very_poor: 'بسیار ناسالم',
        pollutants: 'آلاینده‌ها',
        clouds: 'ابرناکی',
        precip_sky: 'بارش و آسمان',
        precip_trend: 'بارش',
        humidity_trend: 'رطوبت و پوشش ابر',
        no_precip: 'در این بازه بارشی ثبت نشده است.',
        forecast: 'برآورد روند کوتاه‌مدت',
        temp_forecast: 'دما — ساعت‌های آینده',
        aqi_forecast: 'شاخص کیفیت هوا — ساعت‌های آینده',
        forecast_note: 'یک برآورد ساده بر اساس داده‌های اخیر است، نه یک پیش‌بینی هواشناسی.',
        patterns: 'الگوها بر اساس ساعت و روز',
        patterns_note: 'میانگین بر اساس ساعت روز (ستون‌ها) و روز هفته (سطرها)، روی کل تاریخچه جمع‌آوری‌شده.',
        temp_pattern: 'الگوی دما',
        aqi_pattern: 'الگوی شاخص کیفیت هوا',
        health_advice: 'توصیه سلامت',
        main_pollutant: 'آلاینده اصلی در حال حاضر: {p}.',
        aqi_advice_1: 'کیفیت هوا خوب است. زمان مناسبی برای فعالیت در فضای باز برای همه است.',
        aqi_advice_2: 'کیفیت هوا قابل قبول است. افراد بسیار حساس بهتر است فعالیت شدید طولانی در فضای باز را محدود کنند.',
        aqi_advice_3: 'کیفیت هوا متوسط است. گروه‌های حساس (کودکان، سالمندان و افراد دارای بیماری قلبی یا ریوی) فعالیت طولانی در فضای باز را محدود کنند.',
        aqi_advice_4: 'کیفیت هوا ناسالم است. همه باید فعالیت شدید یا طولانی در فضای باز را کاهش دهند؛ گروه‌های حساس در فضای بسته بمانند.',
        aqi_advice_5: 'کیفیت هوا بسیار ناسالم است. از فعالیت در فضای باز خودداری کنید، پنجره‌ها را بسته نگه دارید و در صورت خروج از ماسک یا تصفیه‌کننده هوا استفاده کنید.',
        weekdays: ['دوشنبه', 'سه‌شنبه', 'چهارشنبه', 'پنجشنبه', 'جمعه', 'شنبه', 'یکشنبه'],
    }
};

const CURRENT_LANG = (() => {
    const saved = localStorage.getItem('lang');
    return (saved === 'en' || saved === 'fa') ? saved : 'fa';
})();

function t(key) {
    return (I18N[CURRENT_LANG] && I18N[CURRENT_LANG][key]) || I18N.en[key] || key;
}

function applyLanguage() {
    document.documentElement.lang = CURRENT_LANG;
    document.documentElement.dir = CURRENT_LANG === 'fa' ? 'rtl' : 'ltr';

    document.querySelectorAll('[data-i18n]').forEach(el => {
        el.textContent = t(el.dataset.i18n);
    });

    const toggle = document.getElementById('lang-toggle');
    if (toggle) {
        // The button shows the language you would switch TO
        toggle.textContent = CURRENT_LANG === 'fa' ? 'EN' : 'فارسی';
        toggle.addEventListener('click', () => {
            localStorage.setItem('lang', CURRENT_LANG === 'fa' ? 'en' : 'fa');
            window.location.reload();
        });
    }
}

// ---- Theme (light/dark). Default follows the system preference; the
// navbar toggle overrides it and the choice persists. ----

const CURRENT_THEME = (() => {
    const saved = localStorage.getItem('theme');
    if (saved === 'light' || saved === 'dark') return saved;
    return (window.matchMedia && window.matchMedia('(prefers-color-scheme: light)').matches)
        ? 'light' : 'dark';
})();

function applyTheme() {
    document.documentElement.dataset.theme = CURRENT_THEME;

    const meta = document.querySelector('meta[name="theme-color"]');
    if (meta) meta.content = CURRENT_THEME === 'light' ? '#f6f8fa' : '#0d1117';

    // The switch is static markup; the knob position is driven by the
    // html[data-theme] CSS. Here we only wire the click and label.
    const toggle = document.getElementById('theme-toggle');
    if (toggle) {
        toggle.setAttribute('aria-label', t('theme_toggle'));
        toggle.setAttribute('title', t('theme_toggle'));
        toggle.setAttribute('aria-checked', CURRENT_THEME === 'dark' ? 'true' : 'false');
        toggle.addEventListener('click', () => {
            localStorage.setItem('theme', CURRENT_THEME === 'light' ? 'dark' : 'light');
            window.location.reload();
        });
    }
}

applyLanguage();
applyTheme();
