async function loadWindData() {
    ...
    try {
        ...
        document.getElementById('avg-wind').textContent = fmt(avgSpeed.average_windspeed);
        document.getElementById('peak-wind').textContent = fmt(peakSpeed.peak_windspeed);
        document.getElementById('wind-direction').textContent = fmt(direction.dominant_direction);
        document.getElementById('direction-text').textContent = getWindDirection(direction.dominant_direction);
        document.getElementById('wind-variability').textContent = fmt(variability.direction_variability);
...
async function loadCalmPeriods() {
    ...
    try {
        ...
        document.getElementById('calm-count').textContent = fmt(data.result?.calm_periods);
        document.getElementById('total-periods').textContent = fmt(data.result?.total_periods);
        document.getElementById('calm-percentage').textContent = fmt(data.result?.calm_percentage);
    } catch (error) {
